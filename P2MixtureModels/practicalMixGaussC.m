...
for (cData =1:nData)
    ...
    sigma = mixGauss.cov(:,:,h);
    mean = mixGauss.mean(:,h);
    chol_decomp = chol(sigma, 'lower');
    this_sample = chol_decomp * randn(2,1) + mean;
    
    data(:,cData) = this_sample;
...
for(cData = 1:nData)
    ...
    for (k = 1:mixGaussEst.k);
        this_weight = mixGaussEst.weight(k);
        this_x = data(:,cData);
        this_mean = mixGaussEst.mean(:,k);
        this_cov = mixGaussEst.cov(:,:,k);
        this_like = this_weight * getGaussProb(this_x,this_mean,this_cov);
        like = like + this_like;
    end
...
% Custom-made subroutine to return probability of multivariate Gaussian
% TAKES COLUMN VECTORS.
function prob = getGaussProb(x,mean,cov)
dim = length(mean);
const = 1 / ((2*pi)^(dim/2) * sqrt(det(cov)));
meandiff = x - mean;
power = -1/2 * meandiff' * inv(cov) * meandiff;
prob = const * exp(power);