...
x2 = -3:0.01:3;
[nDim2, nData2] = size(x2);
x2Transform = nonLinearTransform(x2);
% Compute the mean and variance of the prediction for each element of
% x2.
phi_vector = vertcat(offsetEst, slopeEst);

%Concat ones atop the x2Transform data.
ones_to_concat = ones(1,nData2);
x2Transform = vertcat(ones_to_concat,x2Transform);

predMean = phi_vector' * x2Transform;
predVar = repmat(noiseEst,1,nData2);
...
for (cData = 1:nData)
    ...
    %replace this:
    z1 = exp(-(thisX - (-2.5))^2 / (2 * 0.16));
    z2 = exp(-(thisX - (-1.5))^2 / (2 * 0.16));
    z3 = exp(-(thisX - (-0.5))^2 / (2 * 0.16));
    z4 = exp(-(thisX - (0.5))^2 / (2 * 0.16));
    z5 = exp(-(thisX - (1.5))^2 / (2 * 0.16));
    z6 = exp(-(thisX - (2.5))^2 / (2 * 0.16));
    
    
    
    transformedX = [z1, z2, z3, z4, z5, z6]';
