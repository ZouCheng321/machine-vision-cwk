...
% Make into column vector with ones.
nData = size(x2,2);
ones_to_concat = ones(1, nData);
x2_vector = vertcat(ones_to_concat, x2);

% Make parameters vector
phi_vector = [offsetEst slopeEst]';

% Compute estimates and transopse such that they're in a row
% like the x2 data.
predMean = (x2_vector' * phi_vector)';
predVar = (repmat(noiseEst,nData,1))';
...
%Replace this
noiseEst_num = (w - x'*phi)' * (w - x'*phi);
noiseEst_denom = nData;
noiseEst = noiseEst_num / noiseEst_denom;
